...
use crate::kerberos::client::generators::{
    generate_authenticator, generate_final_neg_token_targ, generate_neg_ap_req_plain, get_mech_list,
...
impl<'a> Kerberos {
    ...
    pub(crate) async fn initialize_security_context_impl(
        ...
        match service_name {
            "HTTP" => self.initialize_security_context_impl_plain(yield_point, builder).await,
            _ => {
                self.initialize_security_context_impl_p2p(yield_point, builder, &username, service_name)
                    .await
            }
        }
    }

    pub(crate) async fn initialize_security_context_impl_plain(
        &'a mut self,
        yield_point: &mut YieldPointLocal,
        builder: &'a mut crate::builders::FilledInitializeSecurityContext<'_, <Self as SspiImpl>::CredentialsHandle>,
    ) -> Result<crate::InitializeSecurityContextResult> {
    ...
    pub(crate) async fn initialize_security_context_impl_p2p(
        &'a mut self,
        yield_point: &mut YieldPointLocal,
        builder: &'a mut crate::builders::FilledInitializeSecurityContext<'_, <Self as SspiImpl>::CredentialsHandle>,
        username: &'a str,
        ...
        debug!("use kerberos peer to peer protocol");
        trace!(?builder);
