...
pub async fn accept_security_context(
    ...
    let status = match server.state {
        KerberosState::Preauthentication => {
            ...
            let ticket_decryption_key =
                server_data
                    .ticket_decryption_key_for(ticket_service_name)
                    .ok_or_else(|| {
                        Error::new(
                            ErrorKind::InvalidToken,
                            format!(
            ...
            let authenticator_seq_number: Option<u32> = seq_number.0.as_ref().map(|seq| {
                ...
                let slice = seq_number_bytes.get(start..).expect("start <= seq_number_bytes.len()");
            ...
            let ap_options = u32::from_be_bytes(
                ap_options_bytes
                    .get(1..)
                    .expect("ap_options_bytes.len() == 5 due to prior check")
                    .try_into()
                    .map_err(|err| Error::new(ErrorKind::InvalidToken, format!("invalid ApReq ap-options: {err:?}")))?,
            );
