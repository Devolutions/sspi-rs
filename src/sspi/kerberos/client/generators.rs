...
mod tests {
    ...
    fn test_decrypt_priv() {
        ...
        // println!("{:?}", enc_data);
        let ap_rep_enc = [218, 123, 236, 220, 101, 47, 45, 70, 106, 25, 67, 106, 10, 200, 237, 233, 168, 230, 209, 134, 210, 70, 15, 179, 21, 129, 41, 72, 205, 206, 37, 58, 143, 60, 37, 48, 137, 187, 89, 131, 16, 52, 68, 37, 60, 28, 215, 252, 225, 97, 29, 147, 62, 127, 19, 216];
        let priv_enc = [244, 180, 210, 36, 82, 20, 173, 202, 122, 213, 65, 87, 59, 79, 72, 138, 41, 183, 39, 148, 25, 196, 189, 182, 26, 48, 252, 101, 54, 24, 238, 24, 228, 212, 69, 37, 151, 225, 49, 193, 172, 32, 236, 245, 125, 139, 33, 149, 71, 31, 65, 220, 230, 121, 86];
    ...
    #[test]
    fn just_decryption() {
        let priv_ap_req_auth = [87, 123, 80, 209, 27, 22, 225, 218, 169, 58, 110, 54, 14, 187, 224, 18, 232, 118, 204, 166, 76, 116, 115, 146, 111, 138, 218, 30, 197, 120, 1, 174, 58, 56, 24, 81, 60, 88, 206, 125, 168, 129, 85, 97, 66, 213, 13, 247, 231, 86, 64, 72, 145, 34, 4, 7, 95, 124, 102, 38, 143, 184, 65, 211, 189, 99, 157, 192, 76, 194, 12, 66, 84, 201, 29, 201, 35, 254, 26, 227, 192, 184, 83, 2, 171, 171, 171, 143, 52, 236, 154, 158, 150, 136, 240, 183, 252, 26, 225, 109, 223, 251, 217, 104, 116, 142, 171, 190, 5, 231, 125, 84, 210, 252, 226, 68, 15, 109, 56, 103, 221, 9, 204, 90, 126, 119, 17, 62, 90, 215, 148, 102, 123, 163, 22, 207, 16, 223, 156, 233, 132, 198, 212, 31, 21, 62, 166, 152, 17];
        let req_priv_enc = [227, 153, 23, 92, 116, 208, 149, 167, 133, 156, 83, 120, 61, 244, 208, 71, 148, 226, 67, 12, 203, 159, 64, 52, 3, 77, 106, 55, 244, 129, 63, 221, 126, 27, 139, 59, 90, 98, 152, 123, 237, 212, 151, 87, 204, 28, 135, 185, 236, 145, 182, 24, 28, 30, 58];

        let cipher = new_kerberos_cipher(18).unwrap();

        println!("{:?}", cipher.decrypt(
            &[250, 155, 199, 119, 68, 61, 253, 106, 96, 113, 154, 40, 26, 111, 218, 96, 229, 76, 67, 228, 184, 62, 228, 115, 67, 188, 139, 196, 101, 70, 220, 199],
            11,
            &priv_ap_req_auth
        ));

        println!("{:?}", cipher.decrypt(
            &[181, 92, 42, 214, 110, 66, 70, 16, 48, 7, 231, 228, 79, 63, 158, 113, 222, 119, 246, 102, 12, 199, 143, 116, 6, 94, 148, 74, 37, 75, 223, 166],
            13,
            &req_priv_enc,
        ));
    }
    ...
    fn decrypt_response() {
        ...
        let priv_enc = [110, 93, 233, 53, 121, 177, 70, 64, 164, 21, 248, 221, 202, 167, 84, 234, 181, 57, 69, 184, 56, 152, 242, 218, 16, 205, 97, 89, 128, 72, 105, 142, 28, 79, 179, 126, 218, 186, 59, 21, 77, 130, 241, 105, 170, 184, 163, 154, 61, 150, 165, 224, 174, 67, 253];
        ...
        let auth_key = [79, 82, 141, 77, 42, 171, 103, 122, 130, 201, 188, 7, 136, 24, 26, 200, 35, 24, 99, 99, 134, 241, 138, 168, 88, 209, 229, 156, 99, 60, 123, 195];
