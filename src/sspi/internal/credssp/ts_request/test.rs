...
use crate::{internal::credssp::{CredSspClient, ClientMode, CredSspServer, ClientState, ServerState}, Pku2uConfig};

use super::TsRequest;

fn send_to_server(client_state: ClientState, server: &mut CredSspServer<AuthIdentity>) -> TsRequest {
    match client_state {
        ClientState::ReplyNeeded(ts_request) => {
            match server.process(ts_request).unwrap() {
                ServerState::ReplyNeeded(ts) => ts,
                a => panic!("{:?}", a),
            }
        },
...
pub fn auth() {
    let mut client = CredSspClient::new(
        Vec::new(),
        AuthIdentity {
            username: "AzureAD\\s7@dataans.com".into(),
            password: "wwwWWW222@@@".into(),
            domain: None,
        },
        CredSspMode::WithCredentials,
        ClientMode::Pku2u(Pku2uConfig::default()),
        "TERMSRV/dest.dataans.com".into(),
    ).unwrap();
    let mut server = CredSspServer::new(
        ...
        ClientMode::Pku2u(Pku2uConfig::default_server_config()),
    ).unwrap();

    // nego/preauth
    let ts_request = TsRequest::default();
    let client_state = client.process(ts_request).unwrap();
    println!("=========> NEGO CLIENT PROCESSED");

    let ts_request = send_to_server(client_state, &mut server);
    println!("=========> NEGO SERVER PROCESSED");

    // as exchange
    let client_state = client.process(ts_request).unwrap();
    println!("=========> AS_REQ CLIENT PROCESSED");

    let ts_request = send_to_server(client_state, &mut server);
    println!("=========> AS_REP SERVER PROCESSED");

    // ap exchange
    let client_state = client.process(ts_request).unwrap();
    println!("=========> AP_REQ CLIENT PROCESSED");

    let ts_request = send_to_server(client_state, &mut server);
    println!("=========> AP_REP SERVER PROCESSED");

    // pub key auth
    let client_state = client.process(ts_request).unwrap();
    println!("=========> PUB KEY AUTH CLIENT PROCESSED");

    let ts_request = send_to_server(client_state, &mut server);
    println!("=========> PUB KEY AUTH SERVER PROCESSED");

    println!("wow, we're here");
}
