...
use picky_krb::{
    ...
    gss_api::{NegTokenTarg1, WrapToken},
    messages::{ApRep, ApReq, AsRep, TgsRep},
};
...
use self::{
    client::{
        ...
        generators::{
            ...
            generate_authenticator_for_tgs_ap_req, generate_neg_ap_req, generate_neg_token_init,
            generate_tgs_req,
        },
        AES128_CTS_HMAC_SHA1_96, AES256_CTS_HMAC_SHA1_96,
    },
    config::{KdcType, KerberosConfig},
    encryption_params::EncryptionParams,
    server::extractors::extract_tgt_ticket,
    utils::{serialize_message, utf16_bytes_to_utf8_string},
};
    ...
    fn encrypt_message(
        ...
        match self.state {
            KerberosState::PubKeyAuth => {
                *data.buffer.as_mut() = raw_wrap_token[MAX_SIGNATURE..].to_vec();
                let header = SecurityBuffer::find_buffer_mut(message, SecurityBufferType::Token)?;
                *header.buffer.as_mut() = raw_wrap_token[0..MAX_SIGNATURE].to_vec();
            }
    ...
    fn initialize_security_context_impl(
        ...
        let status = match self.state {
            KerberosState::Negotiate => {
                ...
                output_token.buffer.write_all(&picky_asn1_der::to_vec(
                    &generate_neg_token_init(&format!(
                        "{}.{}",
                        username,
            ...
            KerberosState::Preauthentication => {
                ...
                if as_rep.0.is_ok() {
                    return Err(Error {
                        ...
                        description:
                            "KDC server should not proccess AS_REQ without the pa-pac data"
                                .to_owned(),
                    });
                }
