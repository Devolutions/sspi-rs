...
impl Default for Validators {
    fn default() -> Self {
        Self { as_req: Box::new(|_| {}), tgs_req: Box::new(|_| {}), }
    }
}
...
impl KdcMock {
    ...
    pub fn new(realm: String, keys: HashMap<UserName, Vec<u8>>, users: HashMap<UserName, PasswordCreds>) -> Self {
        Self { realm, keys, users, validators: Default::default() }
    }

    /// Creates a new [KdcMock] with custom Kerberos messages validators.
    pub fn new_with_validators(realm: String, keys: HashMap<UserName, Vec<u8>>, users: HashMap<UserName, PasswordCreds>, validators: Validators) -> Self {
        Self { realm, keys, users, validators }
    }
